...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    ref={ref}
    className={cn('fixed inset-0 z-50 bg-slate-950/55 backdrop-blur-sm sheet-overlay', className)}
    {...props}
  />
));
SheetOverlay.displayName = SheetPrimitive.Overlay.displayName;

const sheetVariants = cva('fixed z-50 flex flex-col bg-white shadow-2xl outline-none sheet-panel', {
  variants: {
    side: {
  ...
  ({ side = 'right', className, children, showClose = true, ...props }, ref) => (
    <SheetPortal>
      ...
      <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
        {children}
        {showClose && (
          <SheetPrimitive.Close className="absolute right-4 top-4 z-10 rounded-full p-2 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700">
            <X className="h-4 w-4" />
...
function SheetTitle({ className, ...props }: React.ComponentPropsWithoutRef<typeof SheetPrimitive.Title>) {
  return <SheetPrimitive.Title className={cn('text-2xl font-bold tracking-tight text-slate-950', className)} {...props} />;
}
...
function SheetDescription({
  ...
  return <SheetPrimitive.Description className={cn('text-sm leading-6 text-slate-500', className)} {...props} />;
}
